  ...
  return (
    ...
    >
      ...
      <div className={`absolute left-8 md:left-1/2 top-8 md:top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-center justify-center rounded-full shadow-lg z-10 transition-all duration-500 ${
        isFocused 
          ? 'w-10 h-10 bg-blue-600 border-4 border-blue-100 dark:border-blue-900/50' 
          : 'w-6 h-6 bg-gray-200 dark:bg-slate-700 border-2 border-white dark:border-slate-800'
      }`}>
        <div className={`bg-white dark:bg-blue-400 rounded-full transition-all duration-500 ${isFocused ? 'w-2 h-2' : 'w-0 h-0'}`}></div>
      </div>

      {/* Date Badge (Desktop) */}
      <div className={`hidden md:block flex-1 ${index % 2 === 0 ? 'text-left' : 'text-right'}`}>
        <div className={`inline-block px-4 py-2 rounded-full text-sm font-bold shadow-sm border transition-all duration-500 ${
          isFocused 
            ? 'bg-blue-600 text-white border-blue-600 scale-105' 
            : 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-white/5'
        }`}>
      ...
      <div className="flex-1 w-full pl-20 md:pl-0">
         <div 
            className={`bg-white dark:bg-slate-900 rounded-xl overflow-hidden border shadow-md transition-all duration-500 cursor-pointer group hover:-translate-y-1 relative flex flex-col sm:flex-row h-full sm:h-auto ${
              isFocused ? 'ring-2 ring-blue-500/20 shadow-xl border-blue-100 dark:border-blue-900/50' : 'border-gray-100 dark:border-white/5'
            }`}
...
const Projects: React.FC = () => {
  ...
  return (
    <section id="projects" className="py-24 bg-white dark:bg-slate-950" aria-label="Portfolio Projects">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-6">
          <div className="text-center md:text-left">
            <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 dark:text-white mb-2">Portfolio Overview</h2>
            <div className="w-20 h-1.5 bg-blue-600 dark:bg-blue-500 rounded-full mb-4 mx-auto md:mx-0" aria-hidden="true"></div>
        ...
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projectsData.map((project) => (
              <div key={project.id} className="project-card bg-white dark:bg-slate-900 rounded-2xl overflow-hidden border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-2xl dark:hover:shadow-blue-900/20 transition-all duration-300 flex flex-col h-full transform hover:-translate-y-2 group">
                <div 
                ...
                >
                  ...
                  <div className="absolute top-4 right-4 bg-white/95 dark:bg-slate-800/95 backdrop-blur px-3 py-1.5 rounded-full text-xs font-bold text-blue-800 dark:text-blue-400 shadow-lg tracking-wide uppercase">
                    {project.category}
                  </div>
                </div>
                ...
                <div className="p-8 flex-1 flex flex-col">
                  {project.client && (
                     <div className="flex items-center gap-1.5 text-xs font-semibold text-blue-600 dark:text-blue-400 mb-2 uppercase tracking-wide">
                  ...
                  <h3 
                    className="text-2xl font-bold text-gray-900 dark:text-white mb-3 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors cursor-pointer" 
                    onClick={() => setSelectedProject(project)}
                  >
                    {project.title}
                  </h3>
                  <p className="text-gray-600 dark:text-gray-400 text-sm mb-6 leading-relaxed line-clamp-3">{project.description}</p>
                  
                  <div className="flex flex-wrap gap-2 mt-auto pt-6 border-t border-gray-100 dark:border-white/5 mb-6">
                    {project.tags.slice(0, 3).map((tag, i) => (
                      <span key={i} className="inline-flex items-center text-xs font-medium text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-slate-800 px-3 py-1.5 rounded-full">
                  ...
                  <button 
                    onClick={() => setSelectedProject(project)}
                    className="w-full mt-auto bg-gray-900 dark:bg-blue-600 text-white font-medium py-3 rounded-xl hover:bg-blue-600 dark:hover:bg-blue-500 transition-colors flex items-center justify-center gap-2 group/btn"
                  >
                    View Case Study
      ...
      {selectedProject && (
        ...
        >
          ...
          <div className="relative bg-white dark:bg-slate-900 md:rounded-2xl w-full max-w-5xl h-full md:max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col animate-[fadeIn_0.3s_ease-out]">
            ...
            <div className="p-6 sm:p-10 lg:p-12">
              <div className="mb-12 border-b border-gray-100 dark:border-white/5 pb-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-100 dark:border-white/5 shadow-sm">
                  <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Challenge</h4>
                  <p className="text-gray-600 dark:text-gray-400 leading-relaxed text-sm">{selectedProject.challenge}</p>
                </div>
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-100 dark:border-white/5 shadow-sm">
                  <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Solution</h4>
                  <p className="text-gray-600 dark:text-gray-400 leading-relaxed text-sm">{selectedProject.solution}</p>
                </div>
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-100 dark:border-white/5 shadow-sm">
                  <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Outcome</h4>
                  <p className="text-gray-600 dark:text-gray-400 leading-relaxed text-sm">{selectedProject.result}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2">
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Key Responsibilities</h3>
                  <ul className="space-y-4">
                    {selectedProject.keyResponsibilities?.map((resp, idx) => (
                      <li key={idx} className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-slate-800 rounded-xl border border-gray-100/50 dark:border-white/5">
                         <CheckCircle2 className="w-4 h-4 text-blue-600 dark:text-blue-400 mt-1 shrink-0" />
                ...
                <div className="lg:col-span-1">
                   <div className="bg-gray-900 dark:bg-slate-800 text-white rounded-2xl p-6 shadow-xl sticky top-8 border border-white/5">
                      ...
                      {selectedProject.link ? (
                         <a 
                           ...
                           target="_blank" 
                           rel="noopener noreferrer"
                           className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3 rounded-xl transition-all shadow-lg"
                         >
                           Visit Project
                      ...
                      <div className="mt-8 flex flex-wrap gap-2">
                        {selectedProject.tags.map((tag, i) => (
                          <span key={i} className="text-[10px] bg-white/10 px-2.5 py-1 rounded-md text-blue-100 border border-white/5 uppercase tracking-widest font-black">
                            {tag}
                          </span>
                        ))}
                      </div>
